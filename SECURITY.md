...
Use isolated browser contexts. Test processes may load an authorized environment file without printing it; never inspect or copy `.env.live`. Do not disable certificate verification. A mocked protocol test does not establish live-node completeness or prove that a deployment is safe for public hosting.
