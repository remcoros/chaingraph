...
} from '../../../src/Core/Workspace/Wallets/walletSelectionIndex';
import {
  RECEIVE_ADDRESS,
  SECOND_ADDRESS,
  PUBLIC_ZPUB,
...
function fixture(): Workspace {
  ...
  return {
    ...
    wallets: {
      ...createWorkspace('Public selection fixture', 'mainnet').wallets,
      definitions: [wallet],
    },
    chainData: {
      ...createWorkspace('Public selection fixture', 'mainnet').chainData,
...
describe('component-owned wallet selection indexes', () => {
  ...
  it('resolves attached prevouts without inventing creating transactions', () => {
    ...
    expect(
      index.scriptTransactionIds.get(workspace.wallets.definitions[0].addresses[0].scripthash),
    ).toEqual([TX_SPENDING]);
  });

  it('excludes conflicting inputs while preserving authoritative loaded-output address facts', () => {
    const workspace = fixture();
    const { n: _, ...prevout } = workspace.chainData.transactions[TX_FUNDING].vout[1];
    workspace.chainData.transactions[TX_SPENDING].vin[1].prevout = { ...prevout, value: 99 };
    const index = compare(workspace);
    expect(index.prevouts.get(`${TX_FUNDING}:1`)?.status).toBe('conflict');
    expect(
      index.scriptTransactionIds.get(workspace.wallets.definitions[0].addresses[1].scripthash),
    ).toEqual([TX_FUNDING]);
  });
  ...
  it('preserves invalid-map-key exclusions and network boundaries', () => {
    ...
    workspace.chainData.transactions['c'.repeat(64)] =
      workspace.chainData.transactions[TX_SPENDING];
    delete workspace.chainData.transactions[TX_SPENDING];
    expect(compare(workspace).transactions.has(TX_SPENDING)).toBe(false);
    workspace.network = 'testnet4';
    const index = compare(workspace);
    expect(
      buildWalletSelectionAddresses(workspace.wallets.definitions[0], workspace.network).addresses
        .size,
    ).toBe(0);
    expect(
  ...
  it('rebuilds observation changes without modifying an earlier snapshot', () => {
    ...
    const incoming: Transaction = structuredClone(workspace.chainData.transactions[TX_SPENDING]);
    const { n: _, ...prevout } = workspace.chainData.transactions[TX_FUNDING].vout[0];
    ...
    const updated = {
      ...
      chainData: {
        ...workspace.chainData,
        transactions: { ...workspace.chainData.transactions, [TX_SPENDING]: incoming },
      },
    };
    const after = compare(updated);
    expect(before.prevouts.get(`${TX_FUNDING}:0`)?.status).toBe('loaded');
    expect(after.prevouts.get(`${TX_FUNDING}:0`)?.status).toBe('conflict');
  });

  it('projects repeated selections without enumerating unrelated loaded transactions or verifying wallet addresses again', () => {
    const workspace = fixture();
    const index = buildWalletSelectionIndex(workspace);
    const addresses = buildWalletSelectionAddresses(
    ...
    const guarded = {
      ...
      chainData: {
        ...workspace.chainData,
        transactions: new Proxy(workspace.chainData.transactions, {
          ownKeys() {
    ...
    const wallet = {
      ...workspace.wallets.definitions[0],
      addresses: new Proxy(workspace.wallets.definitions[0].addresses, {
