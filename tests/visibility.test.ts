...
describe('manual entity visibility', () => {
  ...
  it('counts unique transaction inputs and outputs, excludes coinbase and keeps group actions reversible', () => {
    const w = setNodesHidden(workspace(), [outputNodeId(a, 0)], true);
    expect(transactionNodeIds(funding, 'inputs')).toEqual([]);
    expect(transactionVisibility(w, funding, 'outputs')).toEqual({
      nodeIds: [outputNodeId(a, 0), outputNodeId(a, 1)],
      totalCount: 2,
      hiddenCount: 1,
      visibleCount: 1,
    });
    expect(transactionVisibility(w, spending, 'inputs')).toEqual({
      nodeIds: [outputNodeId(a, 0)],
      totalCount: 1,
      hiddenCount: 1,
      visibleCount: 0,
    });
    const hidden = setNodesHidden(w, transactionNodeIds(funding, 'outputs'), true);
    expect(transactionVisibility(hidden, funding, 'outputs').hiddenCount).toBe(2);
