...
test('workspace password validation stays inline', async ({ page }) => {
  ...
  await expect(dialog.getByText('At least 8 characters. Use a long, unique passphrase.')).toBeVisible();
  ...
  expect(await page.locator('main.welcome').evaluate((el) => el.scrollTop)).toBe(backgroundScroll);
  ...
  await expect(dialog.getByRole('button', { name: 'Hide password', exact: true })).toHaveAttribute(
    'aria-pressed',
    'true',
  );
  ...
  await expect(page.getByRole('button', { name: 'New workspace', exact: true }).last()).toBeFocused();
});
