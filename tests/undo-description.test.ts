...
describe('undo action descriptions', () => {
    ...
    (patch, expected) => {
      ...
      expect(
        describeWorkspaceChange(before, {
          ...before,
          annotations: { 'tx:a': { ...annotation, ...patch } },
        }),
      ).toBe(expected);
    },
  );
  ...
  it('describes removal of the only populated annotation field', () => {
    const before = { ...workspace(), annotations: { 'tx:a': { ...annotation, bookmarked: true } } };
    expect(describeWorkspaceChange(before, { ...before, annotations: {} })).toBe('Remove bookmark');
  });
  ...
  it('does not choose one label for mixed annotation or bookmark edits', () => {
    ...
    const after = {
      ...before,
      annotations: { 'tx:a': annotation, 'tx:b': { ...annotation, bookmarked: true } },
    };
  ...
  it('keeps transaction addition concise when its graph/context and stale findings follow', () => {
    const before = { ...workspace(), findings: [finding] };
    const after: Workspace = {
      ...before,
      transactions: { a: { txid: 'a', vin: [], vout: [] } },
      inputContext: { a: [0] },
      findings: [{ ...finding, stale: true }],
      view: { ...before.view, graphNodeIds: ['tx:a'], selectionId: 'tx:a' },
    };
  ...
  it('describes wallet identity and naming changes without guessing at other wallet edits', () => {
    const before = workspace();
    const added = { ...before, wallets: [wallet] };
    ...
    expect(
      describeWorkspaceChange(added, { ...added, wallets: [{ ...wallet, name: 'Renamed' }] }),
    ).toBe('Rename wallet');
    expect(
      describeWorkspaceChange(added, { ...added, wallets: [{ ...wallet, color: '#000000' }] }),
    ).toBe('Edit workspace');
  });
  ...
  it('distinguishes tag creation, membership edits and removal', () => {
    ...
    const added = { ...before, tags: [tag] };
    expect(describeWorkspaceChange(before, added)).toBe('Add tag');
    expect(
      describeWorkspaceChange(added, { ...added, tags: [{ ...tag, nodeIds: ['tx:a'] }] }),
    ).toBe('Assign tag');
  ...
  it('counts batch annotation changes and distinguishes tag assignment from metadata editing', () => {
    ...
    const tagged = { ...before, tags: [tag] };
    expect(describeWorkspaceChange(tagged, { ...tagged, tags: [{ ...tag, nodeIds: [] }] })).toBe(
      'Remove tag',
    );
    expect(
      describeWorkspaceChange(tagged, { ...tagged, tags: [{ ...tag, nodeIds: ['tx:b'] }] }),
    ).toBe('Change tags');
    expect(
      describeWorkspaceChange(tagged, { ...tagged, tags: [{ ...tag, name: 'Renamed' }] }),
    ).toBe('Edit tag');
  });

  it('distinguishes running analysis from excluding and restoring findings', () => {
    const before = workspace();
    const analysed = { ...before, findings: [finding] };
    const excluded = { ...analysed, findings: [{ ...finding, excluded: true }] };
  ...
  it('uses a broad description for unknown or unrelated mixed changes', () => {
    ...
    expect(
      describeWorkspaceChange(before, {
        ...before,
        name: 'Renamed',
        annotations: { 'tx:a': { ...annotation, label: 'Label' } },
      }),
    ).toBe('Edit workspace');
    expect(
      describeWorkspaceChange(before, { ...before, watchedAddresses: ['public fixture'] }),
    ).toBe('Edit workspace');
  ...
  it('does not inspect unchanged transaction observations for a metadata edit', () => {
    const before = workspace();
    const transactions = new Proxy(before.transactions, {
      ownKeys() {
        throw new Error('Unchanged transactions must not be inspected');
      },
    });
    const source = { ...before, transactions };
