...
describe('address-level wallet relationship subjects', () => {
  it('groups distinct outputs by canonical address without double counting repeated contexts', () => {
    ...
    expect(source).toMatchObject({
      id: `addr:${other}`,
      ownership: 'external',
      outpointIds: [outputNodeId(id(10), 0), outputNodeId(id(10), 1)],
      transactionIds: [id(2), id(3), id(4)],
      amountSats: 600_000_000,
      count: 2,
    });
    expect(source.contexts).toEqual([
      { transactionId: id(2), walletOutputIds: [outputNodeId(id(2), 0), outputNodeId(id(2), 1)] },
      { transactionId: id(3), walletOutputIds: [outputNodeId(id(3), 1)] },
      { transactionId: id(4), walletOutputIds: [outputNodeId(id(4), 0)] },
    ]);
    ...
    expect(destination.outpointIds).toEqual([outputNodeId(id(3), 0)]);
    expect(destination.amountSats).toBe(100_000_000);
    expect(destination.id).toBe(source.id);
    expect(grouped.sourceExceptions.map((entry) => entry.id)).toEqual([outputNodeId(id(9), 3)]);
  ...
  it('keeps malformed, non-address and missing outputs as exceptions rather than inventing groups', () => {
    ...
    expect(grouped.sourceExceptions.map((entry) => entry.id).sort()).toEqual(
      [outputNodeId(id(9), 3), outputNodeId(id(10), 0), outputNodeId(id(10), 1)].sort(),
    );
  ...
  it('preserves wallet/network isolation and ignores conflicting address text when grouping', () => {
    ...
    workspace.transactions[id(10)].vout[0].scriptPubKey.address = mine;
    workspace.transactions[id(10)].vout[1].scriptPubKey = { address: other.toUpperCase() };
    ...
    expect(testnet.sourceExceptions.some((entry) => entry.id === outputNodeId(id(10), 1))).toBe(
      true,
    );
  });
  ...
  it('does not present a partial or unsafe sum as the observed output total', () => {
    const workspace = fixture();
    workspace.transactions = structuredClone(workspace.transactions);
    workspace.transactions[id(10)].vout[1].value = Number.NaN;
    expect(
      groupWalletRelationships(workspace, wallet).sources.find((entry) => entry.address === other)
        ?.amountSats,
    ).toBeUndefined();
    workspace.transactions[id(10)].vout[0].value = Number.MAX_SAFE_INTEGER / 100_000_000;
    workspace.transactions[id(10)].vout[1].value = Number.MAX_SAFE_INTEGER / 100_000_000;
    expect(
      groupWalletRelationships(workspace, wallet).sources.find((entry) => entry.address === other)
        ?.amountSats,
    ).toBeUndefined();
...
describe('direct wallet relationships', () => {
  it('deduplicates canonical subjects while retaining every one-hop context and wallet reference', () => {
    ...
    const direct = sources.find((source) => source.id === outputNodeId(id(10), 0))!;
    expect(direct).toMatchObject({
      ...
      walletOutputIds: [outputNodeId(id(2), 0), outputNodeId(id(2), 1), outputNodeId(id(4), 0)],
    });
    ...
    expect(destinations[0].walletOutputIds).toEqual([outputNodeId(id(2), 0)]);
    expect(destinations[1].ownership).toBe('wallet');
    expect(sources.find((source) => source.id === outputNodeId(id(2), 0))?.ownership).toBe(
      'wallet',
    );
    expect(destinations.some((destination) => destination.id === outputNodeId(id(2), 2))).toBe(
      false,
    );
  ...
  it('uses raw scripts over claims, with malformed, non-address and foreign-address evidence unknown', () => {
    ...
    expect(results.destinations.map((entry) => entry.id)).toEqual(
      receiving.vout.map((entry) => outputNodeId(id(2), entry.n)),
    );
  });
  ...
  it('uses the declared network for raw scripts and rejects inconsistent transaction map keys', () => {
    ...
    wrongKey.transactions[id(99)] = receipt;
