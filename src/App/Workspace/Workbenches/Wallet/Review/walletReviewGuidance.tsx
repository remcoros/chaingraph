...
export function walletReviewGuidance(
  ...
  if (reason === 'link') {
    const organize = canSelectRelated ? (
      <>
        Use <strong>Select related</strong> to select matching results, then <strong>Tags</strong>{' '}
        to record a shared purpose, such as “Donations”.
      </>
    ) : (
      <>
        Use <strong>Tags</strong> to record a purpose you recognize, such as “Donations”.
      </>
    );
    const acknowledge = (
      <>
        Choose <strong>Mark reviewed</strong> when you are done.
      </>
    );
    if (finding?.algorithm.replace(/-v\d+$/, '') === 'address-reuse') {
      ...
      return repeated ? (
        <>
          This address was used in multiple transactions, making those payments easy to connect. Use
          a fresh receiving address for future payments. {organize} {acknowledge}
        </>
      ) : (
        <>
          Several outputs in one transaction use the same address. This does not necessarily mean it
          was reused for separate payments. {organize} {acknowledge}
        </>
      );
    }
    ...
    return (
      <>
        ...
        {finding?.guidance?.text ?? (
          <>
            Open <strong>Transaction flow</strong> and add any labels or tags that help explain the
            activity.
          </>
        )}{' '}
        {acknowledge}
  ...
  const finish = row.reviews.length ? (
    <>
      Confirm the recorded context, then choose <strong>Mark reviewed</strong>.
    </>
  ) : (
