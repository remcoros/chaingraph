...
const transactionStatusSchema = z
  .object({
    kind: z.enum(['confirmed', 'mempool', 'inactive', 'unknown']),
  ...
  .superRefine((transaction, context) => {
    ...
    const confirmed = transaction.blockHeight !== undefined || (transaction.confirmations ?? 0) > 0;
    const inactive = (transaction.confirmations ?? 0) < 0;
    if (
      (transaction.kind === 'confirmed') !== confirmed ||
      (transaction.kind === 'inactive' && confirmed) ||
      (inactive && transaction.kind !== 'inactive')
    )
      context.addIssue({
        code: 'custom',
        message: 'Transaction placement contradicts its status.',
      });
  });
