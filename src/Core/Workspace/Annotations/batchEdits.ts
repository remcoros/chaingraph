...
import { MAX_TAG_MEMBERS, MAX_WORKSPACE_TAGS, type Annotation } from './annotations';
 ...
 */
const MAX_BATCH_TARGETS = 50_000;
...
function targets(workspace: Workspace, ids: Iterable<string>): string[] {
  ...
  for (const id of ids) {
    ...
    result.add(canonicalEntityReference(id, workspace.network));
  }
  return [...result];
}
...
export function labelBatchPlan(
  ...
  const labeled = all.filter((id) => Boolean(workspace.annotations.entities[id]?.label.trim()));
  ...
  return {
    ...
    distinctLabels: [
      ...new Set(labeled.map((id) => workspace.annotations.entities[id]!.label.trim())),
    ].slice(0, 5),
  };
}
...
export function applyBatchLabel(
  ...
  return changed
    ? { ...workspace, annotations: { ...workspace.annotations, entities: annotations } }
    : workspace;
}
...
export function tagMemberCount(workspace: Workspace): number {
  return (workspace.annotations.tags ?? []).reduce((total, tag) => total + tag.nodeIds.length, 0);
}
...
export function applyBatchTag(
  ...
  const all = targets(workspace, ids);
  const tag = (workspace.annotations.tags ?? []).find((entry) => entry.id === tagId);
  ...
  return {
    ...
    annotations: {
      ...
      tags: (workspace.annotations.tags ?? []).map((entry) =>
