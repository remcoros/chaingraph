...
import { satoshiValue } from './tools/shared';
import { analysisScriptType } from './tools/scripts';
import { analysisInputFingerprint } from './analysisScan';

/** Rebase input-only enrichment onto current observations and user edits in one commit. */
export function publishRecoveredAnalysis(
  before: Workspace,
  next: Workspace,
  current: Workspace,
): Workspace {
  if (
    ...
    before.network !== current.network ||
    analysisInputFingerprint(before, Object.keys(before.chainData.transactions)) !==
      analysisInputFingerprint(current, Object.keys(before.chainData.transactions))
  )
    return current;
  let transactions = current.chainData.transactions;
  for (const [id, enriched] of Object.entries(next.chainData.transactions)) {
    if (enriched === before.chainData.transactions[id]) continue;
    const latest = current.chainData.transactions[id];
    if (!latest) return current;
    if (transactions === current.chainData.transactions) transactions = { ...transactions };
    // Recovery obtains previous outputs, never a replacement placement observation.
    transactions[id] = { ...latest, vin: enriched.vin };
  }
  return { ...current, chainData: { ...current.chainData, transactions }, analysis: next.analysis };
}
...
export async function recoverAnalysisData(
  ...
  const snapshot = () => ({
    ...workspace,
    chainData: { ...workspace.chainData, transactions: transactions },
  });
  ...
  function attach(txid: string, incoming: Transaction) {
    ...
    try {
      ...
      const index = indexPreviousOutputs({
        ...
        transactions: snapshot().chainData.transactions,
      });
      for (const input of merged.vin) {
        if (!input.prevout) continue;
        const known = resolvePreviousOutput(
          { network: workspace.network, transactions: workspace.chainData.transactions },
          input,
          index,
        );
  ...
  const changed = Object.keys(transactions).some(
    (id) =>
      JSON.stringify(transactions[id]) !== JSON.stringify(workspace.chainData.transactions[id]),
  );
  return {
