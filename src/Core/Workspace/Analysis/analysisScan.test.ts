...
describe('contextual scan scope', () => {
  ...
  it('isolates an explicit wallet from the active wallet and graph selection', () => {
    ...
    const spent = tx(6, 5);
    workspace.chainData.transactions[id(5)] = received;
    workspace.chainData.transactions[id(6)] = spent;
    workspace.wallets.definitions = [wallet(), otherWallet];
  ...
  it('keeps a removed explicit wallet empty even if a stale wallet and graph selection remain', async () => {
    ...
    workspace.wallets.definitions = [imported];
    ...
    for (const mode of ['context', 'workspace']) {
      for (const selected of [
        ...
        const updated = {
          ...
          analysis: {
            ...workspace.analysis,
            findings: mergeScanFindings(workspace.analysis.findings, scan),
          },
        };
