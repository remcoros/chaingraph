...
import {
  ...
  recoverAnalysisData,
  recoveryLimits,
} from './analysisRecovery';
import { analysisTools } from './analysis';
import { createWorkspace } from '../createWorkspace';
import { type Transaction, resolvePreviousOutput } from '../../ChainData';
...
function workspace(...txs: Transaction[]) {
  ...
  w.chainData.transactions = Object.fromEntries(txs.map((tx) => [tx.txid, tx]));
  return w;
}
const fees = (w: Workspace) => analysisTools.find((tool) => tool.id === 'value-flow')!.run(w);

describe('bounded Analysis missing-data recovery', () => {
  it('publishes enrichment and findings atomically while retaining a newer confirmation and note', () => {
    const before = workspace(spend());
    const enriched = workspace({ ...spend(), vin: [{ txid: id(1), vout: 0, prevout: output }] });
    const next = {
      ...before,
      chainData: enriched.chainData,
      analysis: { findings: fees(enriched) },
    };
    const current = {
      ...before,
      ...
      chainData: {
        ...before.chainData,
        transactions: {
          [id(10)]: { ...spend(), status: { kind: 'confirmed' as const, confirmations: 4 } },
        },
      },
    };
    const published = publishRecoveredAnalysis(before, next, current);
    expect(published.description).toBe(current.description);
    expect(published.chainData.transactions[id(10)].status?.confirmations).toBe(4);
    expect(published.chainData.transactions[id(10)].vin[0].prevout).toEqual(output);
    expect(published.analysis.findings).toBe(next.analysis.findings);
  });
  ...
  it('refreshes the spending transaction at verbosity 2 through the adapter before considering parents', async () => {
    ...
    expect(Object.keys(result.workspace.chainData.transactions)).toEqual([id(10)]);
    expect(result.workspace.view).toBe(w.view);
    expect(result.resolved).toBe(1);
    expect(fees(result.workspace)[0].title).toContain('Fee threshold reached');
    expect(w.chainData.transactions[id(10)].vin[0].prevout).toBeUndefined();
  });
  it('deduplicates shared parents, retains partial success and retries only unresolved data', async () => {
    ...
    const fetch = vi.fn(async (_network, txid: string) => {
      ...
      return w.chainData.transactions[txid] ?? parent(1);
    });
    const result = await recoverAnalysisData(
      w,
      [id(10), id(11)],
    ...
    const retry = vi.fn(
      async (_network, txid: string) => result.workspace.chainData.transactions[txid] ?? parent(2),
    );
    ...
    expect(Object.keys(done.workspace.chainData.transactions)).toHaveLength(2);
  });
  it('retains usable parent amounts even when script details remain unavailable', async () => {
    for (const scripts of [false, true]) {
      ...
      const fetch = vi.fn(
        async (_network, txid: string) => w.chainData.transactions[txid] ?? valueOnly,
      );
      const result = await recoverAnalysisData(
        w,
        [id(10)],
      ...
      expect(result.workspace.chainData.transactions[id(10)].vin[0].prevout?.value).toBe(1);
  ...
  it('fills missing script details from attachments even when the parent is loaded', async () => {
    ...
    expect(fetch).toHaveBeenCalledTimes(1);
    const resolved = resolvePreviousOutput(
      { network: result.workspace.network, transactions: result.workspace.chainData.transactions },
      { txid: id(1), vout: 0 },
    );
  ...
  it('caps concurrency and total unique transaction lookups without deep traversal', async () => {
    ...
    const fetch = vi.fn(async (_network, txid: string) => {
      ...
      active--;
      return w.chainData.transactions[txid] ?? parent(parseInt(txid, 16));
    });
    const result = await recoverAnalysisData(
      w,
      Object.keys(w.chainData.transactions),
      fetch,
      new AbortController().signal,
    );
    ...
    expect(Object.keys(result.workspace.chainData.transactions)).toHaveLength(12);
  });
});
...
it('treats contradictory script-type observations as conflicts without replacing existing evidence', async () => {
  ...
  const fetch = vi.fn(
    async (_network, txid: string) => w.chainData.transactions[txid] ?? parent(2),
  );
  const result = await recoverAnalysisData(w, [id(10)], fetch, new AbortController().signal);
  expect(result.conflicts).toBeGreaterThan(0);
  expect(result.workspace.chainData.transactions[id(10)].vin[0].prevout!.scriptPubKey.type).toBe(
    'witness_v0_keyhash',
  );
...
it('automatic enrichment uses a smaller fixed budget and reuses resolved evidence on the next scan', async () => {
  ...
  const fetch = vi.fn(
    async (_network, txid: string) => w.chainData.transactions[txid] ?? parent(parseInt(txid, 16)),
  );
  const result = await recoverAnalysisData(
    w,
    Object.keys(w.chainData.transactions),
  ...
  expect(fetch.mock.calls.filter((call) => w.chainData.transactions[call[1]])).toHaveLength(4);
  expect(result.budgetReached).toBe(true);
  expect(result.remaining).toBeGreaterThan(0);
  const resolved = workspace(spend());
  resolved.chainData.transactions[id(10)].vin[0].prevout = output;
  fetch.mockClear();
...
it('uses remaining automatic capacity for more spends when verbosity 2 resolves the first batch', async () => {
  ...
  const fetch = vi.fn(async (_network, txid: string) => ({
    ...
    vin: w.chainData.transactions[txid].vin.map((input) => ({ ...input, prevout: output })),
  }));
  const result = await recoverAnalysisData(
    w,
    Object.keys(w.chainData.transactions),
  ...
  expect(Object.keys(result.workspace.chainData.transactions)).toHaveLength(6);
});
