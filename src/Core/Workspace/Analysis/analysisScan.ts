...
import { listWalletTransactions } from '../Wallets/walletRecords';
import { addressToScriptHash } from '../../Bitcoin';
import { findingInputs } from './findingStaleness';
import { stableKey } from './tools/shared';
...
export type AnalysisScopeWorkspace = Pick<Workspace, 'network'> & {
  chainData: Pick<Workspace['chainData'], 'transactions'>;
  wallets: Pick<Workspace['wallets'], 'definitions'>;
};
/** A selected chain subject, independent of how a workbench presents it. */
...
export interface AnalysisScan {
  ...
  runAt: string;
  evidenceTransactions?: Workspace['chainData']['transactions'];
  inputFingerprint?: string;
}

/** Content dependencies of the completed scan, independent of refresh timestamps. */
export function analysisInputFingerprint(workspace: Workspace, txids: string[]): string {
...
export function analysisScanMatches(scan: AnalysisScan, workspace: Workspace): boolean {
  return scan.inputFingerprint === undefined
    ? scan.evidenceTransactions === undefined ||
        scan.evidenceTransactions === workspace.chainData.transactions
    : scan.inputFingerprint === analysisInputFingerprint(workspace, scan.scope.txids);
}
...
export function analysisScopeChoice(
  ...
  const scopeWallet = walletId
    ? workspace.wallets.definitions.find((entry) => entry.id === walletId)
    : undefined;
...
export function analysisScanScope(
  ...
  const loaded = (ids: string[]) =>
    [...new Set(ids)].filter((id) => workspace.chainData.transactions[id]).sort();
  ...
  if (selected?.kind === 'output')
    return {
      ...
      txids: loaded([
        ...
        ...Object.values(workspace.chainData.transactions)
          .filter((tx) =>
            tx.vin.some(
              (input) =>
  ...
  return {
    ...
    txids: Object.keys(workspace.chainData.transactions).sort(),
  };
}
...
export async function scanAnalysis(
  ...
  for (const tool of analysisTools) {
    ...
    try {
      ...
      const unavailable =
        tool.id === 'wallet-intersections' && workspace.wallets.definitions.length < 2;
      reports.push({
        toolId: tool.id,
  ...
  return {
    ...
    evidenceTransactions: workspace.chainData.transactions,
    inputFingerprint: analysisInputFingerprint(workspace, scope.txids),
  };
}
