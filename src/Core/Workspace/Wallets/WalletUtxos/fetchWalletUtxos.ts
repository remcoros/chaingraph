import { type Network, addressToScriptHash } from '../../../Bitcoin';
import type { Wallet } from '../wallets';
import type { WalletUtxoRecord } from '../walletRecords';
import type { AddressUtxoObservation } from '../../../ChainData/observations';
...
export interface WalletUtxoBatch {
  ...
  /** Successful per-address observations, including checked empty results. */
  observations: Record<string, AddressUtxoObservation>;
  checkedAt: string;
...
export async function fetchWalletUtxos(
  ...
  wallet: Wallet,
  options: {
    readAddressUtxos: (address: string, signal?: AbortSignal) => Promise<AddressUtxoObservation>;
    cursor?: number;
    signal?: AbortSignal;
  ...
  await Promise.all(
    Array.from({ length: Math.min(4, slice.length) }, async () => {
      while (next < slice.length) {
        ...
        try {
          const observation = await options.readAddressUtxos(address.address, signal);
          signal?.throwIfAborted();
          if (
            observation.network !== network ||
            observation.utxos.length > MAX_WALLET_UTXOS_PER_ADDRESS
          )
            throw new Error('Invalid address observation.');
          ...
          for (const item of observation.utxos) {
            ...
            if (
              ...
              conflicts.add(key);
              records.delete(key);
              if (previous && result.observations[previous.address]) {
                delete result.observations[previous.address];
                result.errors.push({
          ...
          result.observations[address.address] = observation;
  ...
  signal?.throwIfAborted();
  result.successfulAddresses = Object.keys(result.observations).length;
  result.records = [...records.values()]
    .filter((record) => !!result.observations[record.address])
    .sort(
      (a, b) =>
