...
import { fetchWalletUtxos as readWalletUtxos } from './fetchWalletUtxos';
import { fetchAddressUtxos } from '../../../ChainData/api';

function fetchWalletUtxos(
  network: Network,
  wallet: Wallet,
  options: Omit<Parameters<typeof readWalletUtxos>[2], 'readAddressUtxos'> = {},
) {
  return readWalletUtxos(network, wallet, {
    ...options,
    readAddressUtxos: (address, signal) => fetchAddressUtxos(network, address, signal),
  });
}
...
describe('wallet UTXO observation fetching', () => {
  ...
  it('rejects contradictory duplicate outpoints instead of selecting whichever request arrives first', async () => {
    ...
    expect(acrossAddresses.errors).toHaveLength(2);
    expect(acrossAddresses.successfulAddresses).toBe(0);
    expect(acrossAddresses.observations).toEqual({});
