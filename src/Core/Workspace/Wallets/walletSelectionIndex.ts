...
import { indexPreviousOutputs, type PreviousOutputIndex, type Transaction } from '../../ChainData';
...
export function buildWalletSelectionIndex(
  ...
  for (const [txid, transaction] of transactions) {
    ...
    for (const input of transaction.vin) {
      ...
      const resolved = prevouts.get(`${parent}:${input.vout}`);
  ...
  for (const [key, transaction] of Object.entries(workspace.chainData.transactions)) {
    for (const output of transaction.vout) {
      ...
      ids.push(outpointReference(key, output.n));
