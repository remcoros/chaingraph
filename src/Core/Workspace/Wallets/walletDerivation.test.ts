...
import { addressToScriptHash } from '../../Bitcoin/index';
...
describe('imported wallet ownership claims', () => {
  it('accepts authoritative BIP84 receive/change addresses in arbitrary order', () => {
    const addresses = [
      ...
      .reverse();
    expect(() => verifyDerivedAddresses(zpub, 'mainnet', 'p2wpkh', addresses)).not.toThrow();
  });
  ...
  it('rejects an address from another account even with matching hash and plausible path', () => {
    ...
    expect(() => verifyDerivedAddresses(zpub, 'mainnet', 'p2wpkh', foreign)).toThrow(
      'does not match',
    );
  });
  ...
  it('checks sparse maximum indexes without scanning intervening children', () => {
    ...
    try {
      verifyDerivedAddresses(zpub, 'mainnet', 'p2wpkh', addresses);
  ...
  it('rejects duplicate paths and modified derivation metadata', () => {
    ...
    expect(() => verifyDerivedAddresses(zpub, 'mainnet', 'p2wpkh', [address, address])).toThrow(
      'duplicate',
    );
    expect(() =>
      verifyDerivedAddresses(zpub, 'mainnet', 'p2wpkh', [{ ...address, path: 'account/1/0' }]),
    ).toThrow('does not match');
    expect(() =>
      verifyDerivedAddresses(zpub, 'mainnet', 'p2wpkh', [{ ...address, index: 0x80000000 }]),
    ).toThrow('Invalid');
    expect(() => verifyDerivedAddresses(zpub, 'mainnet', 'p2tr', [])).toThrow('requires');
  });
});
