...
import { addressToScriptHash, sats, type TxOutput } from '../../Bitcoin';
import { canonicalEntityReference, outpointReference } from '../entityReferences';
...
export function loadedWalletTransactions(workspace: {
  chainData: Pick<Workspace['chainData'], 'transactions'>;
}): Map<string, Transaction> {
  const transactions = new Map<string, Transaction>();
  for (const [key, transaction] of Object.entries(workspace.chainData.transactions)) {
...
export function listWalletRelationships(
  ...
  const subject = (txid: string, vout: number, output?: TxOutput): WalletRelationship => {
    ...
    return {
      id: outpointReference(txid, vout),
      txid,
      vout,
  ...
  for (const [txid, walletOutputIds] of spent) {
    for (const output of transactions.get(txid)!.vout) {
      if (!validOutputIndex(output.n)) continue;
      add(destinations, outputs.get(outpointReference(txid, output.n))!, txid, walletOutputIds);
    }
  }
...
export function groupWalletRelationships(
  ...
  const group = (entries: WalletRelationship[]) => {
    ...
    for (const entry of entries) {
      ...
      let id: string;
      try {
        id = canonicalEntityReference(`addr:${entry.address}`, workspace.network);
      } catch {
        exceptions.push(entry);
        continue;
      }
