...
import { fetchTransaction } from '../../ChainData/api';
...
describe('explicit scan action evidence', () => {
  it('reuses loaded and retained evidence offline without mutating the workspace', async () => {
    ...
    options.workspace.chainData.transactions[id(1)] = transaction(1);
  ...
  it('fetches only unique requested transactions with navigation priority and no ancestry walk', async () => {
    ...
    expect(options.workspace.chainData.transactions).toEqual({});
  ...
  it.each(['cached', 'fetched'])(
    ...
    async (source) => {
      const options = fixture();
      if (source === 'cached') options.workspace.chainData.transactions[id(1)] = transaction(2);
