...
import { clearScanRuns, replaceScanRun } from '../../ConnectionScan/updates';
...
import { encryptWorkspace } from '../Codec/encryptedEnvelope';
...
function scanFixture() {
  ...
  store.update(
    ...
    (w) => ({
      ...
      chainData: { ...w.chainData, transactions: { [txid(1)]: transaction(1) } },
    }),
    false,
  );
  const run: ScanRun = {
...
describe('workspace redo', () => {
  ...
  it('keeps current scan dismissals and proof usable across Add, Undo and Redo', () => {
    ...
    store.redo(id);
    expect(session().data.chainData.transactions[txid(2)]).toEqual(transaction(2, 1));
    ...
    store.undo(id);
    expect(session().data.chainData.transactions[txid(2)]).toBeUndefined();
  ...
  it('preserves both stacks and accepted chain data through a background refresh', () => {
    const { store, id, session } = setup();
    store.update(id, (w) => ({ ...w, name: 'Renamed' }));
    ...
    store.update(
      id,
      (w) => ({ ...w, chainData: { ...w.chainData, transactions: { [txid(1)]: transaction(1) } } }),
      false,
    );
    expect(session().history).toHaveLength(1);
    expect(session().redoHistory).toHaveLength(1);
    ...
    expect(session().data.chainData.transactions[txid(1)]).toEqual(transaction(1));
  });
  ...
  it('keeps quiet wallet scan metadata and promoted transaction context current through Redo', () => {
    ...
    store.update(
      ...
      (w) => ({
        ...w,
        wallets: { ...w.wallets, definitions: [wallet] },
        chainData: {
          ...w.chainData,
          transactions: { [txid(1)]: transaction(1) },
          contextTransactionIds: [txid(1)],
        },
        view: { ...w.view, inputContext: { [txid(1)]: [0] } },
      }),
      false,
    );
    store.update(id, (w) => ({ ...w, name: 'Renamed' }));
        ...
        clearContextProvenance(
          {
            ...
            wallets: {
              ...w.wallets,
              definitions: w.wallets.definitions.map((item) => ({
                ...item,
                scannedAt,
                scanComplete: true,
                scanLimit: 200,
    ...
    store.redo(id);
    expect(session().data.description).toBe('Note');
    expect(session().data.wallets.definitions[0]).toMatchObject({
      scannedAt,
      scanComplete: true,
      scanLimit: 200,
      scanGap: 20,
    });
    expect(session().data.view.inputContext).toBeUndefined();
    expect(session().data.chainData.contextTransactionIds).toBeUndefined();
    store.undo(id);
    store.undo(id);
    expect(session().data.wallets.definitions[0].scannedAt).toBe(scannedAt);
    expect(session().data.view.inputContext).toBeUndefined();
    expect(session().data.chainData.contextTransactionIds).toBeUndefined();
    expect(() => parseWorkspace(session().data)).not.toThrow();
  });
