...
import { SCAN_LIMITS } from '../../ConnectionScan/scanPath';
...
} from '../../../../App/Workspace/Workbenches/Graph/ConnectionScan/connectionScanAddition';
import { prepareScanPath } from '../../../../App/Workspace/Workbenches/Graph/ConnectionScan/connectionScanPath';
import { replaceScanRun } from '../../ConnectionScan/updates';
...
function fixture() {
  ...
  workspace.chainData.transactions = { [id(3)]: transaction(3, [4, 5]) };
...
describe('bounded scan context proof', () => {
  ...
  it('adds the union once, with matching counts and no sibling outputs', () => {
    ...
    expect(added.chainData.transactions).toEqual({
  ...
  it('requires missing context proof for full Add while a partial prefix excludes it', () => {
    ...
    expect(prefix.chainData.transactions[id(5)]).toBeUndefined();
  });
  ...
  it('rejects newly loaded context proof contradicting a spender outside both routes', () => {
    const { workspace, result, run, evidence } = fixture();
    workspace.chainData.transactions[id(6)] = {
      ...transaction(6),
  ...
  it.each(['wrong edge', 'negative confirmations', 'prevout conflict'])(
    ...
    (issue) => {
      ...
      if (issue === 'negative confirmations')
        evidence[id(5)].status = { kind: 'inactive', confirmations: -1 };
