...
import type { EnvelopeStorage } from '../Browser/indexedEnvelopeStorage';
import { createBrowserWorkspaceStore } from './workspaceStoreFixture';
import { INLINE_INDEX_LIMIT } from '../Browser/workspaceStorage';
...
describe('encrypted workspace overflow storage', () => {
  it('moves a large envelope out of the public index and reloads, edits, locks and deletes it', async () => {
    ...
    for (let i = 0; i < 120; i++)
      workspace.annotations.entities[`tx:${i.toString(16).padStart(64, '0')}`] = {
        label: '',
    ...
    expect(restored.getSnapshot().unlocked[0].data.annotations.entities).toEqual(
