...
import {
  addScanPath,
  prepareScanPath,
} from '../../../../App/Workspace/Workbenches/Graph/ConnectionScan/connectionScanPath';
import { replaceScanRun, clearScanRuns } from '../../ConnectionScan/updates';

import { buildGraph } from '../../../../App/Workspace/GraphState/graphEvidence';
import { createWorkspace } from '../../createWorkspace';
import { parseWorkspace } from '../../Persistence';
import { projectGraphMembership } from '../../../../App/Workspace/GraphState/graphMembership';
import type { Transaction } from '../../../ChainData';

import { createBrowserWorkspaceStore } from './workspaceStoreFixture';
import { decryptWorkspace } from '../Codec/encryptedEnvelope';
import { validateAndEncryptWorkspace, decryptAndValidateWorkspace } from '../Codec/workspaceCodec';
...
describe('compact connection scan records', () => {
  ...
  it('encrypts a compact round trip and restores running records without altering a save input', async () => {
    ...
    expect(restored.chainData.transactions).toEqual(workspace.chainData.transactions);
  });
  ...
  it('keeps prior results and their proof when starting a new scan', () => {
    ...
    expect(replaced.chainData.transactions).toBe(workspace.chainData.transactions);
  ...
  it('updates streaming results once and prunes only empty older scans', () => {
    ...
    expect(afterEmpty.connectionScans!.evidence).toEqual(evidence);
  });
  ...
  it('adds an exact path or explicit prefix, reveals nodes, and preserves camera and annotations', () => {
    ...
    workspace.annotations.entities[out(1)] = {
      label: 'Public label',
      note: '',
      icon: '',
      bookmarked: true,
    ...
    expect(prefix.chainData.transactions[id(3)]).toBeUndefined();
    expect(prefix.chainData.transactions[id(2)]).toEqual(evidence[id(2)]);
    ...
    expect(added.annotations.entities).toBe(workspace.annotations.entities);
    ...
    expect(clearScanRuns(added).chainData.transactions).toBe(added.chainData.transactions);
    expect(clearScanRuns(added).annotations.entities).toBe(added.annotations.entities);
  ...
  it('promotes scoped evidence so accepted edges are actually rendered without adding siblings', () => {
    const { workspace, result, run, evidence } = fixture();
    workspace.view.inputContext = { [id(1)]: [1] };
  ...
  it('adds a boundary path in one undo action and preserves later presentation changes', () => {
    ...
    expect(restored.chainData.transactions).toEqual(saved.chainData.transactions);
  ...
  it('rejects count and byte overflows with actionable errors', () => {
    ...
    expect(() =>
      parseWorkspace({
        ...workspace,
        connectionScans: { runs: [], evidence: { data: 'x'.repeat(2 * 1024 * 1024) } },
      }),
    ).toThrow('2 MiB');
    expect(() =>
...
describe('scan finding evidence and metadata', () => {
  function findingFixture(finding: NonNullable<ScanResult['finding']>) {
    ...
    if (finding === 'unspendable')
      workspace.chainData.transactions[id(1)].vout[0].scriptPubKey.hex = '6a';
    const result: ScanResult = {
      id: `finding:${finding}`,
  ...
  it('round trips timestamped unspent observations without treating them as current spend status', async () => {
    ...
    workspace.chainData.transactions[id(2)] = transaction(2, 1);
    const saved = replaceScanRun(workspace, run);
  ...
  it('rejects forged natural endpoints, branch counts and metadata combinations', () => {
    for (const finding of ['coinbase', 'unspendable', 'many-inputs', 'many-outputs'] as const) {
      const { workspace, run, result } = findingFixture(finding);
      if (finding === 'coinbase') workspace.chainData.transactions[id(1)] = transaction(1, 8);
      if (finding === 'unspendable')
        workspace.chainData.transactions[id(1)].vout[0].scriptPubKey.hex = '51';
    ...
    invalidCoinbase.workspace.chainData.transactions[id(1)].vin[0].coinbase = 'not script bytes';
  ...
  it('checks the exact shared meeting node while retaining legacy paths without this metadata', () => {
    const { workspace, run } = fixture();
    workspace.chainData.transactions = { [id(2)]: transaction(2, 1), [id(3)]: transaction(3, 1) };
    const result: ScanResult = {
      id: 'shared',
      kind: 'connection',
      relationship: 'shared-ancestor',
  ...
  it('blocks contradictory selected edges and full conflicting findings, but accepts verified prefixes', () => {
    ...
    const changed = {
      ...
      chainData: {
        ...saved.chainData,
        transactions: { ...saved.chainData.transactions, [id(3)]: transaction(3, 8) },
      },
    };
    ...
    const stale = {
      ...
      chainData: {
        ...saved.chainData,
        transactions: {
          ...saved.chainData.transactions,
          [id(2)]: { ...evidence[id(2)], status: { kind: 'inactive' as const, confirmations: -1 } },
        },
      },
    };
  ...
  it('retains only an explicitly disputed terminal edge and adds only a consistent prefix', () => {
    const { workspace, run } = fixture();
    workspace.chainData.transactions = { [id(1)]: transaction(1), [id(3)]: transaction(3, 2) };
    ...
    expect(added.chainData.transactions[id(2)]).toBeUndefined();
  ...
  it('allows rechecks to recategorize an eleventh endpoint within the global result cap', () => {
    const { workspace, run, result } = findingFixture('unspent');
    workspace.chainData.transactions[id(1)].vout = Array.from({ length: 51 }, (_, n) => ({
      n,
      value: 1,
      scriptPubKey: { hex: '51' },
    }));
