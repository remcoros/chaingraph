...
async function benchmark(name: string, workspace: Workspace) {
  ...
  for (let round = -1; round < rounds; round++) {
    ...
    for (const legacy of round % 2 === 0 ? [false, true] : [true, false]) {
      ...
      phase = now();
      const restored = parseWorkspace(decoded);
  ...
  return {
    name,
    wallets: workspace.wallets.definitions.length,
    addresses: workspace.wallets.definitions.reduce(
      (total, wallet) => total + wallet.addresses.length,
      0,
    ),
    transactions: Object.keys(workspace.chainData.transactions).length,
    ...sizes,
...
async function compareNativeCodecs(name: string, workspace: Workspace) {
  const bytes = encoder.encode(JSON.stringify(parseWorkspace(workspace)));
