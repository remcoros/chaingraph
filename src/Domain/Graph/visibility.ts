...
import { outputNodeId, type Network, type Transaction, type Workspace } from '../types';

export const MAX_HIDDEN_NODES = 50_000;
export const hiddenNodeIdsSchema = z.array(z.string().max(200)).max(MAX_HIDDEN_NODES);
export type EntityVisibility = 'visible' | 'hidden' | 'all';
...
export function transactionVisibility(
  workspace: Workspace,
  transaction: Transaction,
  side: 'inputs' | 'outputs',
) {
  const nodeIds = transactionNodeIds(transaction, side);
  const hidden = new Set(workspace.view.hiddenNodeIds ?? []);
  const hiddenCount = nodeIds.filter((id) => hidden.has(id)).length;
  return {
    nodeIds,
    totalCount: nodeIds.length,
    hiddenCount,
    visibleCount: nodeIds.length - hiddenCount,
  };
}
