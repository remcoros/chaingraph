...
The entity list remains the alternative interaction path for keyboard access and WebGL failure. Individual node dragging remains disabled due to the verified upstream pointer-event incompatibility. Camera/layout state is transient, while view settings belong to the workspace. The custom Three.js experiment is deliberately separate until the boundary is integrated with the final feature foundation. See [rendering research and validation](research/graph.md) and the [boundary report](experiments/graph-boundary.md).
