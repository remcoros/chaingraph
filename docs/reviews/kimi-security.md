...
1. **BIP329 label import could erase labels and reject conformant files** (`src/lib/labels.ts`).
   ...
   should not alter these values". Previously a record of a known type without a label aborted the
   whole import, and a record with an empty or whitespace-only label overwrote the existing
   annotation label through the merge in `App.tsx`. Records without a usable label are now skipped
   and counted, matching the existing skip semantics for unsupported types. A leading UTF-8
   byte-order mark (common from Windows editors) is tolerated.
...
2. **Unvalidated `xpub` and `addr` references** (`src/lib/labels.ts`, `src/lib/wallet.ts`).
   ...
   label files. `xpub` references must now decode as one of the six known public SLIP132 version
   bytes (new `isExtendedPublicKey` in `src/lib/wallet.ts`); `addr` references must parse as an
   address on mainnet or testnet4. Transaction/output hex references are accepted case-insensitively
   and normalized to canonical lowercase (as are bech32 address references), and output indexes are
   bounded to uint32.
...
3. **Redundant genesis lookup per Electrum request** (`server/app.ts`). Every Electrum call
   ...
   revalidated by `getblockchaininfo` on every request, so the validated genesis is now cached
   after the first success; failures are never cached.

## Test evidence

- `npx vitest run`: 155 tests pass (baseline 145 + 9 new label boundary tests in
  `tests/labels.test.ts` + 1 genesis caching test in `server/app.test.ts`).
- `npm run build` (tsc --noEmit + vite build) passes; `npx prettier --check` passes.
