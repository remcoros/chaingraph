...
Open **Analysis** and press **Scan** to run every applicable existing tool. Scan scope has two choices: **Selection**, with the current type in parentheses, and **Loaded workspace**. Without an explicit scope choice, it defaults to Selection when a wallet is selected, otherwise Loaded workspace. Selection follows the selected graph transaction, output or address first, then the wallet; the scope description names the actual target. Outputs include their creating transaction and loaded exact spenders. An explicit choice stays with the unlocked workspace through navigation, selection changes and scans. If Selection loses its target, Scan is disabled until you select a wallet or entity, or choose Loaded workspace. An existing selection with no loaded transactions remains empty; it never expands to the workspace. Loaded workspace includes data outside graph filters. Optional settings group each check's parameters. **Load missing input data before scanning**, beside the scan controls, can make bounded read-only requests before scanning; turn it off to use only loaded data. Active loading is shown in the scan status.
