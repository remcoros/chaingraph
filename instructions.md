...
Use the quick input in the center of the workbench bar to load a transaction ID, an address, or an output reference written as `transaction-id:output-index`. Undo, Redo and export sit on the right, with Undo and Redo in the workspace menu on narrow screens. Hover either arrow to see its next action, such as “Undo: Change label” or “Redo: Add path”. A new edit clears Redo. On narrow screens, the input wraps below the workbench tabs. Adding an existing entity selects, reveals and centers it immediately, including while offline. Any requested ancestry or address-history loading can continue once the entity is in view. Select an item in the graph or entity list to inspect its details and available actions.

**Load previous txs**, on a selected transaction, adds one earlier level. For an output, **Open creating tx** opens only the transaction that created that outpoint, loading it if needed. Use the transaction-level action to follow its inputs further. A coinbase transaction has no earlier inputs.
...
Transactions are cubes, outputs are spheres, and optional addresses are diamonds. Hover a node for identifiers, values, available details, and compact actions at the top of its card. Connection lines do not open cards, reducing interruptions in dense graphs. **Load previous level** expands that path; **Edit label / notes** shows the side panels, switches to the Inspector tab, and focuses the editor for that node. The inspector and entity list provide the same tracing workflow without hover.
