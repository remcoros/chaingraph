...
The **Browser QA** workflow is manual only and publishes nothing. Choose `runtime` (the default) or `e2e-smoke`. The E2E suite is a small, deliberately minimal smoke check (does the app render, does a workspace open and show its graph), not per-feature coverage, so it stays cheap to run and cheap to fix as the UI evolves. Plan broader exploratory browser QA when preparing a release or investigating a specific concern; neither the narrow runtime check nor the smoke suite replaces that review.
